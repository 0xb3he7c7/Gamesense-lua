...
ffi.cdef[[
typedef uintptr_t (__thiscall* GetClientEntity_42424242_t)(void*, int);
]]
local function get_ent(index)
    local fn = ffi.cast("GetClientEntity_42424242_t", client.create_interface("client_panorama.dll", "VClientEntityList003"))
    return fn(ffi.cast("void***", client.create_interface("client_panorama.dll", "VClientEntityList003"))[0], index)
end

-- PeekBot
local function peekbot()
...
local menu = {
    main_switch = ui_new_checkbox("RAGE", "Other", "AiPeek Improved"),
    key = ui_new_hotkey("RAGE", "Other", "Peek bot key", true, 0),
    mode = ui_new_combobox("RAGE", "Aimbot", "Detection mode", {"Risky", "Safest"}),
    target = ui_new_combobox("RAGE", "Aimbot", "Detection target", {"Current", "All target"}),
    ...
    tick = ui_new_slider("RAGE", "Other", "Reserve extrapolate tick", 0, 5, 0),
    unlock = ui_new_checkbox("RAGE", "Other", "Unlock camera"),
    segament = ui_new_slider("RAGE", "Other", "Segament", 2, 60, 2),
    radius = ui_new_slider("RAGE", "Other", "Radius", 0, 250, 50),
    depart = ui_new_slider("RAGE", "Other", "Department", 1, 12, 2),
    middle = ui_new_checkbox("RAGE", "Other", "Middle point"),
    limit = ui_new_checkbox("RAGE", "Other", "Max prediction point limit"),
    limit_num = ui_new_slider("RAGE", "Other", "Limit num", 0, 20, 5),
    ...
    color = ui_new_color_picker("RAGE", "Aimbot", "Debugger color", 255, 255, 255, 255)
}
...
-- Desync-Fakewalk
local function desync_fakewalk()
    -- [x]============================[ UI References ]============================[x]
